...
function addShape() {
  ...
  const size = shapes[Math.floor(Math.random() * sizes.length)];
...
function render() {
  ...
  const cirlceElements = shapes.map((shape) => {
    const circleElement = document.createElement("Si");
    circleElement.classList.add(shape.color, shape.size);
    return circleElement;
  });
  circleList.replaceChildren(...cirlceElements)
}
